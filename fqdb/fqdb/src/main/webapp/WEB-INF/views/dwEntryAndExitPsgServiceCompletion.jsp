...
<body>
	<!--search-->
	<div class="search_1">
		<form action="dwEntryAndExitPsgsServiceCompletion" method="post" id="form">
		...
		<table class="table_1" width="100%">
			<thead>
				<tr>
					<td rowspan="2" >航线</td>
					<td rowspan="2" >承运人</td>
					<td colspan="2">航班量（次）</td>
					<td colspan="2">旅客吞吐量（人次）</td>
					<td colspan="2">平均可做率（%）</td>
				</tr>
			...
			<tbody>			   
				<c:forEach items="${dwEntryAndExitPsgsServiceCompletions}" var="dwEntryAndExitPsgsServiceCompletion">
					<tr>
					<c:set var="no" value="${no + 1}" />
					<td>${no}</td>
						<td>${dwEntryAndExitPsgsServiceCompletion.aircraftIATACode}</td>
				        <td>${dwEntryAndExitPsgsServiceCompletion.flightTimes}</td>				        
				        <c:set var="flightTimes" value="${flightTimes + dwEntryAndExitPsgsServiceCompletion.flightTimes}" />
				<td>${dwEntryAndExitPsgsServiceCompletion.aircraftTypePercentage}%</td> 
					</tr>
				</c:forEach>
				<tr>
				<td>总数</td>

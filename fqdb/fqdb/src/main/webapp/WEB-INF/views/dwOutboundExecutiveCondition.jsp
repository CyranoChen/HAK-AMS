...
<body>
	<!--search-->
	<div class="search_1">
		<form action="dwOutboundExecutiveCondition" method="post" id="form">
	...
	<div>
		<table class="table_1" width="100%">
			...
			<tbody>			   
				<c:forEach items="${dwOutboundExecutiveConditions}" var="dwOutboundExecutiveCondition">
					<tr>
					<c:set var="no" value="${no + 1}" />
					<td>${no}</td>
						<td>${dwOutboundExecutiveCondition.airlineIATACode}</td>
				        <td>${dwOutboundExecutiveCondition.flightTimes}</td>				        
				        <c:set var="flightTimes" value="${flightTimes + dwOutboundExecutiveCondition.flightTimes}" />
				<td>${dwOutboundExecutiveCondition.aircraftTypePercentage}%</td> 
					</tr>
				</c:forEach>
				<tr>
				<td>总数</td>

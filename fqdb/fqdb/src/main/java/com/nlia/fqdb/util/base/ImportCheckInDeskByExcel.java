...
@Component
public class ImportCheckInDeskByExcel{
	 ...
	 * @throws IOException
	 */
	public List<CheckInDesk> ImportCheckInDeskByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateCheckInDeskFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			CheckInDesk checkInDeskError = new CheckInDesk();
			checkInDeskError.setVerifyDescription(CheckInDeskMessages.getString("COLUMNNAME_ERROR"));
			checkInDeskList.add(checkInDeskError);
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			CheckInDesk checkInDesk = ValidateCheckInDeskFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (checkInDesk.getVerifyDescription() == null || checkInDesk.getVerifyDescription().equals("")) {
					checkInDeskList.add(checkInDesk);
				} else {
					// 如果记录校验失败，清空list列表，然后将校验失败的这条记录加入到列表中
					isDataError = true;
					checkInDeskList.clear();
					checkInDeskList.add(checkInDesk);
				}
			} else {
				if (checkInDesk.getVerifyDescription() != null && !checkInDesk.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<CheckInDesk> ImportCheckInDeskByExcel2007(InputStream inputStream) throws IOException {

...
@Component
public class ImportTaxiwayByExcel{
	 ...
	 * @throws IOException
	 */
	public List<Taxiway> ImportTaxiwayByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateTaxiwayFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			Taxiway taxiwayError = new Taxiway();
			taxiwayError.setVerifyDescription(TaxiwayMessages.getString("COLUMNNAME_ERROR"));
			taxiwayList.add(taxiwayError);
			return taxiwayList;
		}
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			Taxiway taxiway = ValidateTaxiwayFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (taxiway.getVerifyDescription() == null || taxiway.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<Taxiway> ImportTaxiwayByExcel2007(InputStream inputStream) throws IOException {

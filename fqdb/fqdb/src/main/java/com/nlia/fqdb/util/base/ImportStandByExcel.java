...
@Component
public class ImportStandByExcel{
	 ...
	 * @throws IOException
	 */
	public List<Stand> ImportStandByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateStandFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			Stand standError = new Stand();
			standError.setVerifyDescription(StandMessages.getString("COLUMNNAME_ERROR"));
			standList.add(standError);
			return standList;
		}
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			Stand stand = ValidateStandFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (stand.getVerifyDescription() == null || stand.getVerifyDescription().equals("")) {
					standList.add(stand);
				} else {
	 ...
	 * @throws IOException
	 */
	public List<Stand> ImportStandByExcel2007(InputStream inputStream) throws IOException {

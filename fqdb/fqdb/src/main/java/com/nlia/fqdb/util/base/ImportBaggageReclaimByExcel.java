...
@Component
public class ImportBaggageReclaimByExcel{
	 ...
	 * @throws IOException
	 */
	public List<BaggageReclaim> ImportBaggageReclaimByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateBaggageReclaimFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			BaggageReclaim baggageReclaimError = new BaggageReclaim();
			baggageReclaimError.setVerifyDescription(BaggageReclaimMessages.getString("COLUMNNAME_ERROR"));
			baggageReclaimList.add(baggageReclaimError);
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			BaggageReclaim baggageReclaim = ValidateBaggageReclaimFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (baggageReclaim.getVerifyDescription() == null || baggageReclaim.getVerifyDescription().equals("")) {
					baggageReclaimList.add(baggageReclaim);
				} else {
					// 如果记录校验失败，清空list列表，然后将校验失败的这条记录加入到列表中
					isDataError = true;
					baggageReclaimList.clear();
					baggageReclaimList.add(baggageReclaim);
				}
			} else {
				if (baggageReclaim.getVerifyDescription() != null && !baggageReclaim.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<BaggageReclaim> ImportBaggageReclaimByExcel2007(InputStream inputStream) throws IOException {

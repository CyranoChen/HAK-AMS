...
@Component
public class ImportChargeUnitByExcel {
	 ...
	 * @throws IOException
	 */
	public List<ChargeUnit> ImportChargeUnitByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateChargeUnitFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			ChargeUnit chargeUnitError = new ChargeUnit();
			chargeUnitError.setVerifyDescription(ChargeUnitMessages.getString("COLUMNNAME_ERROR"));
			chargeUnitList.add(chargeUnitError);
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			ChargeUnit chargeUnit = ValidateChargeUnitFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (chargeUnit.getVerifyDescription() == null || chargeUnit.getVerifyDescription().equals("")) {
					chargeUnitList.add(chargeUnit);
				} else {
					// 如果记录校验失败，清空list列表，然后将校验失败的这条记录加入到列表中
					isDataError = true;
					chargeUnitList.clear();
					chargeUnitList.add(chargeUnit);
				}
			} else {
				if (chargeUnit.getVerifyDescription() != null && !chargeUnit.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<ChargeUnit> ImportChargeUnitByExcel2007(InputStream inputStream) throws IOException {

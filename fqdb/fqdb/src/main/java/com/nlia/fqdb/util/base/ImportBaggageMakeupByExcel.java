...
@Component
public class ImportBaggageMakeupByExcel{
	 ...
	 * @throws IOException
	 */
	public List<BaggageMakeup> ImportBaggageMakeupByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateBaggageMakeupFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			BaggageMakeup baggageMakeupError = new BaggageMakeup();
			baggageMakeupError.setVerifyDescription(BaggageMakeupMessages.getString("COLUMNNAME_ERROR"));
			baggageMakeupList.add(baggageMakeupError);
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			BaggageMakeup baggageMakeup = ValidateBaggageMakeupFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (baggageMakeup.getVerifyDescription() == null || baggageMakeup.getVerifyDescription().equals("")) {
					baggageMakeupList.add(baggageMakeup);
				} else {
					// 如果记录校验失败，清空list列表，然后将校验失败的这条记录加入到列表中
					isDataError = true;
					baggageMakeupList.clear();
					baggageMakeupList.add(baggageMakeup);
				}
			} else {
				if (baggageMakeup.getVerifyDescription() != null && !baggageMakeup.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<BaggageMakeup> ImportBaggageMakeupByExcel2007(InputStream inputStream) throws IOException {

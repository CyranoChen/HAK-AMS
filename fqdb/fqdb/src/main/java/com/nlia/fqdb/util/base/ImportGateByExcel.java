...
@Component
public class ImportGateByExcel{
	 ...
	 * @throws IOException
	 */
	public List<Gate> ImportGateByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateGateFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			Gate gateError = new Gate();
			gateError.setVerifyDescription(GateMessages.getString("COLUMNNAME_ERROR"));
			gateList.add(gateError);
			return gateList;
		}
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			Gate gate = ValidateGateFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (gate.getVerifyDescription() == null || gate.getVerifyDescription().equals("")) {
					gateList.add(gate);
				} else {
	 ...
	 * @throws IOException
	 */
	public List<Gate> ImportGateByExcel2007(InputStream inputStream) throws IOException {

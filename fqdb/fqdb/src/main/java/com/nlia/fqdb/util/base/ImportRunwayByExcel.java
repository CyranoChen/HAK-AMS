...
@Component
public class ImportRunwayByExcel{
	 ...
	 * @throws IOException
	 */
	public List<Runway> ImportRunwayByExcel2003(InputStream inputStream) throws IOException {
		...
		for (int i = 0; i < columnsCount; i++) {
			...
			// 判断对应列名是否相同
			if (!ValidateRunwayFromExcel.verifyColumnName(i, columnName)) {
				isColumnNameError = true;
				break;
			}
		...
		if (isColumnNameError) {
			Runway runwayError = new Runway();
			runwayError.setVerifyDescription(RunwayMessages.getString("COLUMNNAME_ERROR"));
			runwayList.add(runwayError);
			return runwayList;
		}
		...
		for (int i = 1; i < totalData + 1; i++) {
			...
			// 对获取的数据进行校验，并转化成实体对象
			Runway runway = ValidateRunwayFromExcel.validateSingleData(rowValue, i);

			if (isDataError == false) {
				if (runway.getVerifyDescription() == null || runway.getVerifyDescription().equals("")) {
	 ...
	 * @throws IOException
	 */
	public List<Runway> ImportRunwayByExcel2007(InputStream inputStream) throws IOException {

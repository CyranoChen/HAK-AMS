package com.wonders.frame.ams.bean;

/**
 * Created by 3701 on 2016/3/8.
 */
public class FlightLoadDataBean {
    
    private String id;
    private String hd;
    private String hdfl;
    private String zdyz;
    private String zdzw;
    private String peyz;
    private String pezw;
    private String kgyz;
    private String kgzw;
    private String io;
    private String jcn;
    private String qjsj;
    private String cr;
    private String et;
    private String ye;
    private String crwh;
    private String etwh;
    private String yewh;
    private String xl;
    private String yj;
    private String hw;
    private String wjhz;
    private String xljs;
    private String legno;
    private String generateMethod;




    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getHd() {
        return hd;
    }

    public void setHd(String hd) {
        this.hd = hd;
    }

    public String getHdfl() {
        return hdfl;
    }

    public void setHdfl(String hdfl) {
        this.hdfl = hdfl;
    }

    public String getZdyz() {
        return zdyz;
    }

    public void setZdyz(String zdyz) {
        this.zdyz = zdyz;
    }

    public String getZdzw() {
        return zdzw;
    }

    public void setZdzw(String zdzw) {
        this.zdzw = zdzw;
    }

    public String getPeyz() {
        return peyz;
    }

    public void setPeyz(String peyz) {
        this.peyz = peyz;
    }

    public String getPezw() {
        return pezw;
    }

    public void setPezw(String pezw) {
        this.pezw = pezw;
    }

    public String getKgyz() {
        return kgyz;
    }

    public void setKgyz(String kgyz) {
        this.kgyz = kgyz;
    }

    public String getKgzw() {
        return kgzw;
    }

    public void setKgzw(String kgzw) {
        this.kgzw = kgzw;
    }

    public String getIo() {
        return io;
    }

    public void setIo(String io) {
        this.io = io;
    }

    public String getJcn() {
        return jcn;
    }

    public void setJcn(String jcn) {
        this.jcn = jcn;
    }

    public String getQjsj() {
        return qjsj;
    }

    public void setQjsj(String qjsj) {
        this.qjsj = qjsj;
    }

    public String getCr() {
        return cr;
    }

    public void setCr(String cr) {
        this.cr = cr;
    }

    public String getEt() {
        return et;
    }

    public void setEt(String et) {
        this.et = et;
    }

    public String getYe() {
        return ye;
    }

    public void setYe(String ye) {
        this.ye = ye;
    }

    public String getCrwh() {
        return crwh;
    }

    public void setCrwh(String crwh) {
        this.crwh = crwh;
    }

    public String getEtwh() {
        return etwh;
    }

    public void setEtwh(String etwh) {
        this.etwh = etwh;
    }

    public String getYewh() {
        return yewh;
    }

    public void setYewh(String yewh) {
        this.yewh = yewh;
    }

    public String getXl() {
        return xl;
    }

    public void setXl(String xl) {
        this.xl = xl;
    }

    public String getYj() {
        return yj;
    }

    public void setYj(String yj) {
        this.yj = yj;
    }

    public String getHw() {
        return hw;
    }

    public void setHw(String hw) {
        this.hw = hw;
    }

    public String getWjhz() {
        return wjhz;
    }

    public void setWjhz(String wjhz) {
        this.wjhz = wjhz;
    }

    public String getXljs() {
        return xljs;
    }

    public void setXljs(String xljs) {
        this.xljs = xljs;
    }

    public String getLegno() {
        return legno;
    }

    public void setLegno(String legno) {
        this.legno = legno;
    }

    public String getGenerateMethod() {
        return generateMethod;
    }

    public void setGenerateMethod(String generateMethod) {
        this.generateMethod = generateMethod;
    }
}

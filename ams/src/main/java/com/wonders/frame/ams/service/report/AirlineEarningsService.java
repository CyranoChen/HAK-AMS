...
@Service
public class AirlineEarningsService {
    ...
    public List<Map<String,Object>> query(String startTime,String endTime){
    	String sql = "SELECT "
    			...
    			+ "  FROM V_AIRLINEEARNINGS T WHERE 1 = 1 ";
    	if((startTime!=null&&!("").equals(startTime))&&(endTime!=null&&!("").equals(endTime))){
			sql = sql + " and to_date(t.scheduled_time,'yyyy-MM-dd') between to_date(?,'yyyy-MM-dd') and to_date(?,'yyyy-MM-dd') ";
		}
    	sql += "GROUP BY T.AIRLINE_NAME,T.AIRLINE_IATA_CODE,T.AIRLINE_ICAO_CODE,T.ROUTE_NATURE";
    	sql += " ORDER BY T.ROUTE_NATURE,T.AIRLINE_ICAO_CODE";

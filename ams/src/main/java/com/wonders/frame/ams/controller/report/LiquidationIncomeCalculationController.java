...
import com.wonders.frame.ams.service.report.LiquidationIncomeCalculationService;


@Controller
@RequestMapping("report")
public class LiquidationIncomeCalculationController {
	
	@Resource
	private LiquidationIncomeCalculationService s;
	
	@Resource
	private ExportService es;
	
	
	@RequestMapping("liquidationIncomeCalculation")
    public String index(){
        return "/report/liquidationIncomeCalculation";
    }
	
	@ResponseBody
    @RequestMapping("queryLiquidationIncomeCalculation")
    public List<Map<String,Object>> query(HttpServletRequest request,String startTime,String endTime){
        ...
        session.setAttribute("lstartTime", startTime);
        session.setAttribute("lendTime", endTime);
        return list;
    }
	
	@ResponseBody
	@RequestMapping("lExport")
	public void exportExcel(HttpServletRequest request,HttpServletResponse response) {
		HttpSession session = request.getSession();
		List l = new ArrayList();
		String startTime = session.getAttribute("lstartTime").toString();
		String endTime = session.getAttribute("lendTime").toString();
...
//				+ " AND T.FLIGHT_PROPERTY <> 'Z/X')";
		...
		l.add(l3);
		es.exportExcel(response, l, "liquidationIncomeCalculation", time);
	}
}

...
import com.wonders.frame.ams.service.report.ChargeCalculationService;
...
public class ChargeCalculationController {
	...
	@Resource
	private ChargeCalculationService s;
    ...
    public String index(){
        return "/report/chargeCalculation";
    }
	
	@ResponseBody
    @RequestMapping("queryChargeCalculation")
	public List<Map<String,Object>> query(HttpServletRequest request,String startTime,String endTime){
        ...
        session.setAttribute("cstartTime", startTime);
        session.setAttribute("cendTime", endTime);
        return list;
    }
	...
	public void exportExcel(HttpServletRequest request,HttpServletResponse response) {
		...
		String startTime = session.getAttribute("cstartTime").toString();
		String endTime = session.getAttribute("cendTime").toString();
...
//				+ " AND T.FLIGHT_PROPERTY <> 'Z/X')";
		...
		l.add(l3);
		es.exportExcel(response, l, "chargeCalculation", time);
	}
}
